...
export default function BearCard({ data, isWinner = false, animationDelay = 0 }: BearCardProps) {
  return (
    <div
      className={`report-card debate-slide-right border-r-4 border-r-rose-500 ${
    ...
    >
      <div className="report-card-header">
        <div className="report-card-title">
          <div className="w-9 h-9 rounded-lg bg-rose-50 border border-rose-200 flex items-center justify-center">
            <span className="text-rose-600 font-bold text-sm">B−</span>
          </div>
          <div>
            <span>Bear Analyst</span>
            <p className="text-xs font-normal text-slate-500 mt-0.5">Risk & downside specialist</p>
          </div>
        </div>
        <span
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <span className="font-mono text-2xl font-bold text-rose-700">{data.confidence}%</span>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {data.strongest_arguments.map((arg, i) => (
              <li
                key={i}
                className="flex items-start gap-2 text-sm text-slate-700 bg-rose-50/50 p-2.5 rounded-lg border border-rose-100"
              >
        ...
        <div>
          <p className="report-subheading">Key Risks</p>
          <div className="flex flex-wrap gap-1.5">
            {data.risks.map((risk, i) => (
              <span
                key={i}
                className="text-xs px-2 py-1 bg-rose-50 border border-rose-200 text-rose-800 rounded-md"
              >
                {risk}
              </span>
            ))}
